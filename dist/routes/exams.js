...
router.get('/', auth, (async (req, res) => {
    try {
        ...
        const availableExams = await db.select().from(exams)
            .where(and(lt(exams.startTime, now), gt(exams.endTime, now)));
...
router.get('/results', auth, (async (req, res) => {
    try {
        ...
        const completedSessions = await db.select({
            ...
            .from(examSessions)
            .where(and(eq(examSessions.userId, userId), eq(examSessions.completed, true)))
            .leftJoin(exams, eq(examSessions.examId, exams.id));
        const results = await Promise.all(completedSessions.map(async (session) => {
...
router.post('/:examId/start', auth, (async (req, res) => {
    try {
        ...
        const completedSession = await db.select()
            .from(examSessions)
            .where(and(eq(examSessions.examId, examId), eq(examSessions.userId, userId), eq(examSessions.completed, true)))
            .limit(1);
        ...
        const existingSession = await db.select()
            .from(examSessions)
            .where(and(eq(examSessions.examId, examId), eq(examSessions.userId, userId), eq(examSessions.completed, false)))
            .limit(1);
